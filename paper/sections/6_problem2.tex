% !TeX root = ../main.tex
\section{问题二：基于联合信念状态的生产决策}

\subsection{任务、信息结构与目标}
...
以最终交付一件合格成品为终止条件。设从信息状态 $b$ 出发的最小期望成本为 $V(b)$，售价为 $S$，则初始状态下的期望利润为
\begin{equation}
  \Pi=S-V(b_0).
  \label{eq:q2-profit}
\end{equation}
...
\end{equation}
作为状态，其中 $\mathcal I$ 汇总已有采购、检测、装配、退换和拆解信息，并满足
\[
...
\subsection{动作、成本与状态转移}

\subsubsection{缺件状态的购件动作}
...
\begin{align}
  ...
  \label{eq:q2-bellman}
\end{align}
其中 Bellman 最优方程为有限状态马尔可夫决策过程的标准最优性条件\cite{mdp-puterman}。

程序从 $b_0$ 出发遍历可达信念状态，将概率保留六位小数后合并近似相同的状态，再以 Gauss--Seidel 次序执行值迭代；相邻两轮最大状态价值更新量低于 $10^{-10}$ 时停止。最终重新计算所有状态的 Bellman 残差，并输出状态策略表、各动作 $Q$ 值及运行诊断。这里的策略 $\pi^*(b)$ 随信息状态变化，固定七个二元决策只是它的受限特例。

\subsection{最优策略与期望收益}

为便于把状态策略还原为业务动作，表~\ref{tab:q2-best-policies}列出从初始状态沿最优策略到首次装配的动作，以及首次装配出现不合格时的下一动作。图~\ref{fig:q2-policy-path}用同一口径比较六种情形。
...
图~\ref{fig:q2-policy-path}把每个格子限定为一个阶段动作：颜色编码动作类别，文字给出具体部件或成品处置方式。
...
情形 1、2、4、5 与受限固定策略的成本在数值容差内一致，情形 6 也仅有舍入差异。情形 3 则利用状态相关动作把期望成本由 $39.4111$ 元降为 $39.3467$ 元，降低 $0.0644$ 元：首次对两类部件均不检测，通过成品检测识别不合格成品；拆解后再依据联合后验优先检测部件 1。该差异说明，成品质量观测能够改变后续检测价值，固定全流程变量不足以表达这一信息反馈。

\subsection{收敛、离散精度与参数敏感性}

为检验数值求解，图~\ref{fig:q2-validation}(a) 给出六种情形的最大状态价值更新量；图~\ref{fig:q2-validation}(b) 比较不同信念概率保留位数下的期望成本。
...
六种情形在 $23$ 至 $29$ 轮内达到停止阈值；最终 Bellman 残差最大为 $2.594\times10^{-11}$。以保留七位小数的结果为参照，六位结果的最大绝对成本差为 $3.227\times10^{-6}$ 元，低于正文保留四位小数所对应的分辨率。因此，六位信念合并不会改变当前报告精度下的成本或利润结论。

进一步分别把三类次品率同步缩放、调换损失和拆解成本乘以 $0.8$ 至 $1.2$，每次只改变一类参数。图~\ref{fig:q2-sensitivity}展示期望利润变化；虚线表示题目基准参数。
...
次品率扰动对利润的影响最显著：在给定网格内，情形 2 的利润范围为 $9.1053$ 至 $14.6190$ 元，情形 5 为 $12.9673$ 至 $18.0453$ 元；情形 1、3、5、6 还出现策略切换。调换损失扰动只使情形 5 在倍率 $1.1$ 和 $1.2$ 时转为检测成品；拆解成本扰动仅在情形 3 的倍率 $1.2$ 触发购检部件 1 的策略。图中近似水平的线表示相应最优动作已经隔离了该项损失，而不是参数在模型中被遗漏。

\subsection{本问结论}

问题二的最终输出是信念状态策略而非固定七维向量。六种情形的初始动作、首次装配与缺陷后的后续动作见表~\ref{tab:q2-best-policies}，期望利润依次为 $18.9222$、$12.0000$、$16.6533$、$14.7500$、$15.4500$ 和 $21.6787$ 元。收敛残差、信念离散精度和三组单因素扰动共同支持上述结果在题目参数及其邻域内的数值可信度；策略切换结果同时表明，超出该范围时必须重新求解，不能机械沿用基准动作。
